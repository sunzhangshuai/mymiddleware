...
// ErrOutOfBrokers is the error returned when the network has run out of brokers to talk to because all of them errored
// or otherwise failed to respond.
var ErrOutOfBrokers = errors.New("kafka: network has run out of available brokers to talk to")
...
// ErrClosedClient is the error returned when a method is called on a network that has been closed.
var ErrClosedClient = errors.New("kafka: tried to use a network that was closed")

// ErrIncompleteResponse is the error returned when the server returns a syntactically valid res, but it does
// not contain the expected information.
var ErrIncompleteResponse = errors.New("kafka: res did not contain all the expected topic/partition blocks")
...
// ErrTxnUnableToParseResponse when res is nil
var ErrTxnUnableToParseResponse = errors.New("transaction manager: unable to parse res")
...
// PacketDecodingError is returned when there was an error (other than truncated data) decoding the Kafka broker's res.
...
func (err KError) Error() string {
	...
	case ErrRequestTimedOut:
		return "kafka server: Request exceeded the user-specified time limit in the req"
	case ErrBrokerNotAvailable:
		return "kafka server: Broker not available. Not a network facing error, we should never receive this!!!"
	...
	case ErrNetworkException:
		return "kafka server: The server disconnected before a res was received"
	...
	case ErrInvalidTopic:
		return "kafka server: The req attempted to perform an operation on an invalid topic"
	case ErrMessageSetSizeTooLarge:
		return "kafka server: The req included message batch larger than the configured segment size on the server"
	...
	case ErrInconsistentGroupProtocol:
		return "kafka server: The provider group message type is incompatible with the other members"
	...
	case ErrTopicAuthorizationFailed:
		return "kafka server: The network is not authorized to access this topic"
	case ErrGroupAuthorizationFailed:
		return "kafka server: The network is not authorized to access this group"
	case ErrClusterAuthorizationFailed:
		return "kafka server: The network is not authorized to send this req type"
	...
	case ErrInvalidRequest:
		return "kafka server: This most likely occurs because of a req being malformed by the network library or the message was sent to an incompatible broker. See the broker logs for more details"
	...
	case ErrListenerNotFound:
		return "kafka server: There is no listener on the leader broker that matches the listener on which metadata req was processed"
	case ErrTopicDeletionDisabled:
		return "kafka server: Topic deletion is disabled"
	case ErrFencedLeaderEpoch:
		return "kafka server: The leader epoch in the req is older than the epoch on the broker"
	case ErrUnknownLeaderEpoch:
		return "kafka server: The leader epoch in the req is newer than the epoch on the broker"
	case ErrUnsupportedCompressionType:
		return "kafka server: The requesting network does not support the compression type of given partition"
