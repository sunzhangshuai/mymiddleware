...
- **日志**：每个副本对应一个日志。每个日志对应一个目录。目录下有多个日志分段。
- **日志分段**：由一个数据文件和索引文件组成。数据文件存储的是消息的真正内容，索引文件存储的是数据文件的索引信息。

#### 写入

- 一个日志由多个日志分段组成，日志管理了所有日志分段。
- 日志用 `segments` 保存每个日志分段的基准偏移量到日志分段的映射关系。
...
- 索引文件保存消息偏移量和消息在数据文件中的物理位置。
- 索引文件中索引条目的键存储值是：消息的绝对偏移量减去基准偏移量。
